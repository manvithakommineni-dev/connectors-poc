import type { FacebookItemDetail } from "../api/facebook";

interface Props {
  item: FacebookItemDetail;
}

export function FacebookMetadataPanel({ item }: Props) {
    ...
    <div style={{ padding: "0 4px" }}>
      ...
      {isFieldCatalog && !rawFields && (
        <div style={{ background: "#fff", border: "1px solid #e8eaed", borderRadius: 8, padding: 16, fontSize: 13 }}>
          Official Facebook Graph API field — use in fields= query on Page, Post, Photo, or Video objects.
        </div>
      )}
    </div>
  );
