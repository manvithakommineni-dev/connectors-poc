import type { PinterestItemDetail } from "../api/pinterest";

interface Props {
  item: PinterestItemDetail;
}

export function PinterestMetadataPanel({ item }: Props) {
    ...
    <div style={{ padding: "0 4px" }}>
      ...
      {isFieldCatalog && !rawFields && (
        <div style={{ background: "#fff", border: "1px solid #e8eaed", borderRadius: 8, padding: 16, fontSize: 13 }}>
          Official Pinterest API v5 field — use in Ads or organic API requests.
        </div>
      )}
    </div>
  );
