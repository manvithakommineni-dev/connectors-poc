import type { YouTubeItemDetail } from "../api/youtube";

interface Props {
  item: YouTubeItemDetail;
}

export function YouTubeMetadataPanel({ item }: Props) {
    ...
    <div style={{ padding: "0 4px" }}>
      ...
      {isFieldCatalog && !rawFields && (
        <div style={{ background: "#fff", border: "1px solid #e8eaed", borderRadius: 8, padding: 16, fontSize: 13 }}>
          Official YouTube Data API v3 field — use in part=snippet, statistics, or contentDetails queries.
        </div>
      )}
    </div>
  );
