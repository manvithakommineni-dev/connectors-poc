...
export function AdjustExplorer() {
  const [selectedCategory, setSelectedCategory] = useState<string>("apps");
  const [selectedItem, setSelectedItem] = useState<string | null>(null);
  const [search, setSearch] = useState("");

  const connectQuery = useQuery({
    queryKey: ["adjust-connect"],
    queryFn: adjustApi.connect,
    retry: false,
  });

  const categoriesQuery = useQuery({
    queryKey: ["adjust-categories"],
    queryFn: adjustApi.categories,
    enabled: connectQuery.isSuccess,
  });

  const itemsQuery = useQuery({
    queryKey: ["adjust-items", selectedCategory],
    queryFn: () => adjustApi.items(selectedCategory),
    enabled: connectQuery.isSuccess && !!selectedCategory,
  });

  const detailQuery = useQuery({
    queryKey: ["adjust-detail", selectedCategory, selectedItem],
    queryFn: () => adjustApi.itemDetail(selectedItem!, selectedCategory),
      ...
      >
        <div style={{ padding: "14px 16px", borderBottom: "1px solid #e0e0e0", background: "#fff" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 14, color: "#00A86B" }}>Adjust</span>
          </div>
          <div style={{ fontSize: 10, color: "#64748b", marginTop: 2 }}>Mobile Attribution</div>
        ...
        {(categoriesQuery.data ?? []).map((cat: AdjustCategory) => (
          <button
            key={cat.id}
            onClick={() => {
            ...
            style={{
              ...
              cursor: "pointer",
              textAlign: "left",
              background: selectedCategory === cat.id ? "#e6f9f0" : "transparent",
              color: selectedCategory === cat.id ? "#00A86B" : "#333",
        ...
        {connectQuery.data && (
          ...
          >
            <div style={{ fontWeight: 600 }}>{connectQuery.data.apps_count} apps</div>
            <div>{connectQuery.data.events_count} events</div>
            {connectQuery.data.sample_app && <div>{connectQuery.data.sample_app}</div>}
          </div>
        )}
      </div>
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {connectQuery.isError && (
            <div style={{ padding: 16, fontSize: 12, color: "#c62828", lineHeight: 1.6 }}>
              <strong>Setup required for live Adjust data:</strong>
              <ol style={{ paddingLeft: 18, marginTop: 8 }}>
                <li>Sign up free at adjust.com (Base plan)</li>
                <li>Add a mobile app and integrate the Adjust SDK</li>
                <li>Copy API Token from Account settings → My profile</li>
                <li>Set ADJUST_API_TOKEN in backend/.env and restart backend</li>
              </ol>
              {setupError && (
          ...
          {filteredItems.map((item: AdjustItem) => (
            <button
              key={item.name}
              ...
              style={{
                ...
                background: selectedItem === item.name ? "#e6f9f0" : "#fff",
                cursor: "pointer",
                textAlign: "left",
              }}
            >
              <div
                style={{
                  fontWeight: 600,
                  fontSize: 13,
                  color: selectedItem === item.name ? "#00A86B" : "#1a1a1a",
                }}
              >
        ...
        {!selectedItem && (
          ...
          >
            ...
            <div style={{ fontSize: 18, fontWeight: 600, color: "#333", marginBottom: 8 }}>
              Adjust Metadata Explorer
            </div>
            <div style={{ fontSize: 13, maxWidth: 440, lineHeight: 1.6 }}>
              Live metadata from Adjust Report Service API — apps, events, metrics, dimensions,
              networks, and countries. No demo mode.
            </div>
          </div>
        )}

        {detailQuery.data && <AdjustMetadataPanel item={detailQuery.data} />}
      </div>
    </div>
  );
}
